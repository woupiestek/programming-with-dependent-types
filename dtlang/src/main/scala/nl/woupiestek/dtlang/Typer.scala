...
case class Typer[T](value: Context => Option[T]) {
  ...
  def flatMap[U](f: T => Typer[U]): Typer[U] = Typer {
    context =>
      value(context).flatMap { t =>
        f(t).value(context)
      }
  }
}
...
object Context {
  ...
  case class Grab(
      annotations: Seq[(Expression, Expression)],
      subcontext: Context
  ) extends Context {
    override def get(index: Int): Option[Expression] =
      if (index < annotations.length)
        Some(annotations(index)._1)
      else subcontext.get(index - annotations.length)

    override def getType(index: Int): Option[Expression] =
      if (index < annotations.length)
        Some(annotations(index)._2)
      else subcontext.getType(index - annotations.length)
  }

}

case class Memoized[X, Y](f: (X => Y) => X => Y)
    extends (X => Y) {
...
object Typer {
  ...
  def infer: Expression => Typer[Expression] =
    Memoized[Expression, Typer[Expression]] { infer =>
      {
        case Variable(i) =>
          Typer { context =>
            context getType i
          }
        case Application(y, z) =>
          Typer { context =>
        ...
        case Abstraction(y, z) =>
          Typer { context =>
            infer(z)
              .value(Context.Abstraction(y, context)) map { w =>
              Abstraction(y, w)
            }
          }
        case _ =>
  ...
  def grab(operator: Expression, context: Context): Expression =
    operator match {
      case Variable(i) => context.get(i) getOrElse Variable(i)
      case Application(x, y) =>
        Application(grab(x, context), y.map(grab(_, context)))
      case Abstraction(x, y) =>
        Abstraction(
          x.map(grab(_, context)),
          grab(y, Context.Abstraction(x, context))
        )
  ...
  def normalize(
  ...
  ): Option[Expression] = operator match {
    case Variable(_)       => Some(Application(operator, operands))
    ...
    case Abstraction(x, y) if x.length <= operands.length =>
      ...
      if (check(annotations))
        normalize(
          grab(y, Context.Grab(annotations, Context.Empty)),
          tail
        )
      else None
    case Abstraction(x, y) if x.length > operands.length =>
      ...
      if (check(annotations)) {
        val context = Context.Abstraction(
          tail.reverse,
          Context.Grab(annotations, Context.Empty)
        )
  ...
  def check(
      annotations: Seq[(Expression, Expression)]
  ): Boolean = annotations.forall {
    case (x, y) =>
      infer(x).value(Context.Empty).exists(unify(_, y))
  }

  def unify(x: Expression, y: Expression): Boolean = {
    (normalize(x, Nil), normalize(y, Nil)) match {
      case (
          Some(Application(x1, x2)),
          Some(Application(y1, y2))
          ) =>
        unify(x1, y1) && (x2 zip y2).forall {
          case (x3, y3) => unify(x3, y3)
        }
      case (
          Some(Abstraction(x1, x2)),
          Some(Abstraction(y1, y2))
          ) =>
        unify(x2, y2) && (x1 zip y1).forall {
          case (x3, y3) => unify(x3, y3)
        }
      case (Some(Product(x1)), Some(Product(y1))) =>
        unify(x1, y1)
      case (x0, y0) => x0 == y0
    }
  }
