...
import nl.woupiestek.andrej.parser.{ DTLGrammar, PrintDTL, Rule, StringParser }
...
class ParseTest extends FunSpec {
  ...
  describe("parsing & printing") {
    val appl = application(get(2), List(get(0), application(get(1), List(get(0)))))
    ...
    it("should handle let bindings") {
      ...
      val test4 = Lamb.fold(lets, PrintDTL)(xyz)
      info(test4)
