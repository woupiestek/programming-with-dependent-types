...
import nl.woupiestek.andrej.parser.StringParser
import org.scalatest.FunSpec

class JsonTest extends FunSpec {

  private val grammar = new JsonGrammar(PrintJson)
  private val rule = grammar.value

  describe("the json grammar") {

    it("fail on certain strings") {
      val tests = List("{\"a\":", "{\"\":", "[\"\\", "[-2.]", "[1.]", "[0.e1]", "[2.e+3]", "[2.e-3]", "[2.e3]", "[\"\\a\"]", "\\UA66D")
      val errors = tests.filter(s => StringParser.parse(rule, s).isRight)
    ...
    it("fail on certain strings 2") {
      ...
      val errors = tests.map(s => (s, StringParser.parse(rule, s))).collect { case (x, Right(y)) => x + " <=> " + y }
      info(errors.mkString(" "))
      assert(errors.isEmpty)
    }
    ...
    it("succeed on certain strings") {
      val tests = List("null", "false", "42", "-0.1", "true", "\" \"", "\"asd\"", "\"\"", "[0e1]", "[0e+1]", "{\"foo\u0000bar\": 42}")
      val errors = tests.filter(s => StringParser.parse(rule, s).isLeft)
      info(errors.mkString(" "))
      assert(errors.isEmpty)
    }
