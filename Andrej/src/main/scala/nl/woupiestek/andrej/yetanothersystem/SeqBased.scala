...
case class ArL(index: Int, arg: SbExpression)
    extends SbStatement {
  override def push(
      i: Int,
      value: SbExpression,
...
case class Return(index: Int) extends SbExpression {
  ...
  override def handle(arg: SbExpression): SbExpression =
    Sequence(ArL(index, arg), Return(0))
}

case class Sequence(
    statement: SbStatement,
    expression: SbExpression
) extends SbExpression {
  override def push(
      index: Int,
      value: SbExpression
  ): SbExpression = statement.push(index, value, expression)

  override def handle(arg: SbExpression): SbExpression =
    statement.handle(arg, expression)
}
