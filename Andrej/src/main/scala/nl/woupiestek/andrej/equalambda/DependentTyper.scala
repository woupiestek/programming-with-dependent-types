...
case class Uni(left: DTerm, right: DTerm, term: DTerm)
    extends DTerm

case class Che(left: DTerm, right: DTerm, term: DTerm)
    extends DTerm
...
object DependentTyper {
  ...
  case class QTy(left: Cod, right: Cod, context: List[Dom])
      extends Q

  case class QTe(left: DTerm, right: DTerm, context: List[Dom])
      extends Q

  def rules(term: DTerm): (Cod, List[Dom], Int) => List[Q] =
    term match {
  ...
  private def check(
      ...
      r: DTerm,
      t: DTerm
  )(cod: Cod, context: List[Dom], i: Int): List[Q] =
    QTe(l, r, context) :: rules(l)(Par(i), context, i + 1) ++ rules(
      r
    )(Par(i), context, i + 1) ++
  ...
  private def unify(
    ...
    QTy(Arrow(Equa(l, r), cod), Par(i), context) :: rules(l)(
      Par(i),
      context,
      i + 2
    ) ++
      rules(r)(Par(i), context, i + 2) ++ rules(t)(
      Par(i + 1),
      Equa(l, r) :: context,
      i + 2
    )
  ...
  private def abst(
    ...
    QTy(
      Arrow(Pair(n, Par(i)), Par2(i + 1, Var(n))),
      cod,
      context
    ) ::
      rules(b)(
        Par2(i + 1, Var(n)),
        Pair(n, Par(i)) :: context,
        i + 2
      )
  ...
  private def appl(
    ...
    QTy(Par2(i + 1, y), cod, context) :: rules(x)(
      Arrow(Pair(s"$i", Par(i)), Par2(i + 1, Var(s"$i"))),
      context,
      i + 2
    ) ++
  ...
  private def vari(
      name: String
  ): (Cod, List[Dom], Int) => List[Q] =
    (cod: Cod, context: List[Dom], _: Int) =>
  ...
  def solve2(
      ...
      equations2: List[Y],
      index: Int
  ): Map[Int, U] = equations match {
    case Nil => assignment1
    case X(U(Par(a), _), b, d) :: c =>
      if (assignment1.contains(a))
        solve2(
          assignment1,
          assignment2,
          X(assignment1(a), b, d) :: c,
          equations2,
          index
        ) //incidence check!
      else
    ...
    case X(U(Arrow(a, b), c), U(Arrow(d, e), f), l) :: g =>
      (a, d) match {
        case (Pair(h, i), Pair(j, k)) =>
          solve2(
            assignment1,
            assignment2,
            X(U(i, c), U(k, f), l) :: X(
