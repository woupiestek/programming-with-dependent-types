...
object Strict {
  ...
  case class Appl(head: Variable, args: List[Value])
      extends Value {
    override def buildWith(
        operands: List[Value],
        vars: List[Variable]
    ): TagFree[Effect, Value] =
      execute(Get(head)).flatMap {
        case Some(w) => w.buildWith(args ++ operands, vars)
        case None =>
          pure(
            vars.foldLeft[Value](Appl(head, operands))(
              (v0, v1) => Abst(v1, v0)
            )
          )
      }
  ...
  case class Abst(head: Variable, body: Value) extends Value {
    override def buildWith(
      ...
      operands match {
        ...
        case h :: t =>
          execute(Put(head, h))
            .flatMap(_ => body.buildWith(t, vars))
      }
  }
  ...
  case class Put(key: Variable, value: Value)
      extends Effect[Unit]

  case class Get(key: Variable) extends Effect[Option[Value]]

  case class Unify(left: Value, right: Value)
      extends Effect[Unit]

  //this is a bit underwhelming...
  case class Test(left: Value, right: Value)
  ...
  def reduce(): TermWithQ[Builder] = new TermWithQ[Builder] {
    override def check(
      ...
      (operands: List[Value], vars: List[Variable]) =>
        for {
          ...
          r <- right.build()
          b <- execute(Test(l, r))
          t <- if (b) term.buildWith(operands, vars)
          else execute(Fail)
        } yield t
    }
    ...
    override def application(
      ...
      (operands2: List[Value], vars: List[Variable]) =>
        for {
