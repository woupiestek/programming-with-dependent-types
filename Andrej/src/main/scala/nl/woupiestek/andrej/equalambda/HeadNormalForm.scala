...
trait HeadNormalForm[H, T] {
  ...
  def where(t: T, tasks: Tasks)(implicit T: Term[T], S: NumVar[T]): T = T.where(t, tasks.mapValues(_.value))

  def comp(f: Tasks, g: Tasks): Tasks = g ++ f.mapValues(_.compose(g))

  abstract class Builder(implicit T: Term[T], S: NumVar[T]) {
    def value:T
  ...
  class Index(int: Int)(implicit T: Term[T], S: NumVar[T]) extends Builder {
  ...
  case class Task(builder: Builder, subs: Tasks) {
    def value(implicit T: Term[T], S: NumVar[T]): T = where(builder.value, subs)
  ...
  implicit def reduce(implicit T: Term[T], S: NumVar[T]): Term[Builder] = new Term[Builder] {
