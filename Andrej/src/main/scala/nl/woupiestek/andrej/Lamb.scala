...
object Lamb {
  ...
  case class Appl(operator: Lamb, operands: List[Lamb])
      extends Lamb
  ...
  def fold[E](l: Lamb, e: DeBruijnExpr[E]): E = l match {
    case Vari(i) => e.get(i)
    case Appl(x, y) =>
      e.application(fold(x, e), y.map(fold(_, e)))
    case Subs(x, y) =>
      y.foldLeft(fold(x, e)) {
        case (a, b) => e.push(fold(b, e), a)
      }
  ...
  def product(dom: Lamb, body: Lamb, context: List[Lamb]) =
    Some(Subs(Prod(dom, body), context))

  def tuple(index: Int, args: List[Lamb]) =
    Some(Appl(Vari(index), args))

  def closure(dom: Lamb, body: Lamb, context: List[Lamb]) =
    Some(Subs(Abst(dom, body), context))

  object Instance extends DeBruijnExpr[Lamb] {
    override def get(index: Int): Lamb = Vari(index)

    override def push(value: Lamb, context: Lamb): Lamb =
      context match {
    ...
    override def lambda(dom: Lamb, value: Lamb): Lamb =
      Abst(dom, value)
  ...
  def reduce(
      ...
      context: List[Lamb],
      args: List[Lamb]
  ): Option[Lamb] = pivot match {
    case Vari(i) if i < context.length =>
      reduce(context(i), Nil, args)
    case Vari(i) if i >= context.length =>
      tuple(i - context.length, args)
    case Appl(operator, operands) =>
      reduce(
        operator,
        context,
        operands.map(Subs(_, context)) ++ args
      )
    case Subs(term, context2) =>
      reduce(term, context2 ++ context, args)
    case Abst(dom, body) =>
      args match {
        case head :: tail if typeCheck(dom, head) =>
          reduce(body, head :: context, tail)
  ...
  def typeCheck(dom: Lamb, head: Lamb): Boolean = {
    @tailrec def typeCheck2(
      ...
      case Appl(x, y) =>
        traverse(y)(z => typeOf(z, cont)) match {
          case None => false
          case Some(a) =>
            typeCheck2(x, cont, a.foldLeft(typ) {
              case (b, c) => Prod(b, c)
            })
        }
      case Subs(x, y) =>
      ...
      case Prod(x, y) if unifiable(typ, Univ) =>
        typeCheck2(y, x :: cont, Univ)
      case _ => false
    }
  ...
  def unifiable(x: Lamb, y: Lamb): Boolean = {
    @tailrec def g(z: List[(Lamb, Lamb)]): Boolean = {
      def f(x: Lamb, y: Lamb): Option[List[(Lamb, Lamb)]] =
        for {
          xx <- reduce(x, Nil, Nil)
          yy <- reduce(y, Nil, Nil)
          zz <- (xx, yy) match {
            case (Appl(a, b), Appl(c, d))
                if a == c && b.length == d.length =>
              Some(b.zip(d))
            case (Subs(Abst(a, b), c), Subs(Abst(d, e), f)) =>
              Some(
                (Subs(a, c), Subs(d, f)) :: (
                  Subs(b, c),
                  Subs(e, f)
            ...
            case (Subs(Prod(a, b), c), Subs(Prod(d, e), f)) =>
              Some(
                (Subs(a, c), Subs(d, f)) :: (
                  Subs(b, c),
                  Subs(e, f)
  ...
  def typeOf(term: Lamb, cont: List[Lamb]): Option[Lamb] = {
    @tailrec def project(
      ...
      (tp, args) match {
        case (Some(Subs(Prod(a, b), c)), head :: tail)
            if typeCheck(head, a) =>
          project(Subs(b, c), tail)
  ...
  private def traverse[X, Y](
      x: List[X]
  )(f: X => Option[Y]): Option[List[Y]] = x match {
    case Nil => Some(Nil)
    case h :: t =>
      for {
        h2 <- f(h)
