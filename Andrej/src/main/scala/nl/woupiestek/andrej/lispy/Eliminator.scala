...
object Eliminator {
  ...
  def eliminate(
    ...
    case Product(entries) =>
      ...
      Some(Product(es))
    case Select(Var(x), y, z) =>
      context.get(x).flatMap {
        case Product(entries) =>
          for {
            y2 <- entries.get(y)
            z2 <- eliminate(z, context + (x -> y2))
        ...
        case _ =>
          eliminate(z, context).map(z2 => Select(Var(x), y, z2))
      }
    case Lambda(Var(x), y) =>
      eliminate(y, context - x)
        .map(z => Lambda(Var(x), z)) //risk of variable capture!
    case Apply(Var(x), y, z) =>
      context.get(x).flatMap {
        case Lambda(Var(a), b) =>
          for {
            y2 <- eliminate(y, context)
  ...
  case class Task(expr: Expr, context: Map[String, Task]) {
    def next: Option[Task] = expr match {
      case Var(x) => context.get(x)
      case Let(Var(x), y, z) =>
        Some(Task(z, context + (x -> Task(y, context))))
      case Product(_) => None
      case Select(Var(x), y, z) =>
        context.get(x).flatMap {
          case Task(Product(entries), context2) =>
            entries.get(y).map { y2 =>
              Task(z, context + (x -> Task(y2, context2)))
            }
          case task =>
            task.next.map(x2 => Task(expr, context + (x -> x2)))
        }
