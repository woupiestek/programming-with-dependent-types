...
class JsonGrammar[J](j: Value[J]) {
  ...
  private val space: R[Unit] = filter[Option[Char]](
    _.exists(jsonWhites.contains)
  ).zeroOrMore.map(_ => ())

  private val char = collect[Option[Char], Char] {
    case Some(c) => c
  }

  private def matchChar(c: Char) =
    filter[Option[Char]](_.contains(c))

  private def token(a: Char): R[Unit] = matchChar(a) ~ space

  private def matchString[X](string: String, x: => X): R[X] =
    matchList[Option[Char]](string.map(Some(_)).toList) ~ space
      .map(_ => x)
  ...
  def string: R[String] = {
    ...
    val simpleEscape: R[Char] = collect[Option[Char], Char] {
      case Some(x) if escapes.contains(x) => escapes(x)
    }
    val hex: Map[Char, Int] =
      ((0 to 9).map(i => ('0' + i).toChar -> i) ++
        (0 to 6).flatMap(
          i =>
            List(
              ('a' + i).toChar -> (10 + i),
    ...
    val hexDigit: R[Int] = collect[Option[Char], Int] {
      case Some(x) if hex.contains(x) => hex(x)
    }
    ...
    ): R[java.lang.StringBuilder] = char.flatMap {
      case '"' =>
        Point[Option[Char], java.lang.StringBuilder](builder)
      case '\\' =>
        (simpleEscape.map(builder.append) | unicodeEscape.map(
  ...
  def number: R[J] = {
    val ncs: Map[Char, Int] =
      (0 to 9).map(i => ('0' + i).toChar -> i).toMap
    val digit = collect[Option[Char], Int] {
      case Some(c) if ncs.contains(c) => ncs(c)
    }

    val exponent: Rule[Option[Char], Int] = for {
      _ <- matchChar('e') | matchChar('E')
      s <- (matchChar('-') ~ Point(-1)) | (matchChar('+').zeroOrOne ~ Point(
        1
      ))
      d <- digit.oneOrMore.map(_.foldLeft(0) {
        case (x, y) => 10 * x + y
      })
    } yield s * d
    ...
    val fraction: Rule[Option[Char], Double] = for {
      _ <- matchChar('.')
      a <- digit.oneOrMore.map(_.foldRight(0.0) {
        case (x, y) => 0.1 * (x + y)
      })
    } yield a
    ...
    val natural: Rule[Option[Char], Int] = for {
      ...
      m <- if (n == 0) Point(0)
        ...
        digit.zeroOrMore.map(_.foldLeft(n) {
          case (x, y) => 10 * x + y
        })
    } yield m
    ...
    val positive: Rule[Option[Char], Double] = for {
      ...
      _ <- space
    } yield (m + p) * math.pow(10.0, q.toDouble)

    positive.map(j.number) | (matchChar('-') ~ positive.map(
      d => j.number(d * -1)
    ))
  }

  def array: R[J] = {
    def inhabited(elts: List[J]): R[List[J]] =
      for {
        elt <- value
        result <- (token(']') ~ Point(elt :: elts)) | (token(
          ','
        ) ~ inhabited(elt :: elts))
      } yield result
  ...
  def jObject: R[J] = {
    def inhabited(pairs: Map[String, J]): R[Map[String, J]] =
      for {
        key <- string
        _ <- token(':')
        value <- value
        next = pairs + (key -> value)
        result <- (token(',') ~ inhabited(next)) | (token('}') ~ Point(
          next
        ))
      } yield result
  ...
  def value: R[J] =
    jTrue | jFalse | jNull | array | jObject | number | string
      .map(j.string)
}
...
object PrintJson extends Value[String] {
  ...
  override def boolean(boolean: Boolean): String =
    boolean.toString

  override def array(array: List[String]): String =
    array.mkString("[", ", ", "]")

  override def jObject(obj: Map[String, String]): String =
    obj
      .map {
