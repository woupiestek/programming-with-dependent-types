...
case class SimpleType(sources: List[SimpleType], target: Int) {
  def ->:(source: SimpleType): SimpleType =
    copy(sources = source :: sources)

  def parameters: Set[Int] =
    sources.flatMap(_.parameters).toSet + target
...
object SimpleType {
  ...
  object % {
    ...
    def unapply(arg: SimpleType): Option[Int] =
      Some(arg.target).filter(_ => arg.sources.isEmpty)
  }

  object ->: {
    def unapply(
        arg: SimpleType
    ): Option[(SimpleType, SimpleType)] = arg.sources match {
      case Nil    => None
