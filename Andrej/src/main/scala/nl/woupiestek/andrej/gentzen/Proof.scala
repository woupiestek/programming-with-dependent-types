...
object Context {
  ...
  case class Pushed(head: List[Proof], tail: Context = Empty)
      extends Context {
    override def get(index: Int): Proof =
      head lift index getOrElse (tail get (index - head.length))
  }

  case class Shifted(amount: Int, tail: Context = Empty)
      extends Context {
    override def get(index: Int): Proof =
...
case class Copy(index: Int, continuation: Proof) extends Proof {
  override def applied(values: List[Proof]): Proof =
    Copy(index, continuation.applied(values))

  override def reduce(context: Context): Proof =
    Push(List(Antecedent(index)), continuation).reduce(context)
}

//arrow introductions
case class ArrowRight(arity: Int, continuation: Proof)
    extends Proof {
  override def applied(values: List[Proof]): Proof =
    if (values.length >= arity) {
      ...
      ArrowRight(
        arity - values.length,
        continuation.reduce(Context.Pushed(values))
      )

  override def reduce(context: Context): Proof =
    ArrowRight(
      arity,
      continuation.reduce(Context.Shifted(arity, context))
    )
}

case class ArrowLeft(
    index: Int,
    arguments: List[Proof],
    continuation: Proof
) extends Proof {
  override def applied(values: List[Proof]): Proof =
    ArrowLeft(index, arguments, continuation.applied(values))
...
//cut rule
case class Push(values: List[Proof], continuation: Proof)
    extends Proof {
  override def applied(values2: List[Proof]): Proof =
    Push(values, continuation.applied(values2))
