...
case class FDefunction(index: Int, arg: FTask, cont: FTask)
    extends FTask

object Typing extends FExpr[List[FTask] => FTask] {
  override def get(index: Int): (List[FTask]) => FTask =
    _ lift index getOrElse FFail

  override def push(
      value: (List[FTask]) => FTask,
      cont: (List[FTask]) => FTask
  ): (List[FTask]) => FTask =
    list => cont(value(list) :: list)

  override def function(
      dom: (List[FTask]) => FTask,
      cont: (List[FTask]) => FTask
  ): (List[FTask]) => FTask =
    list => {
      val d = dom(list)
  ...
  override def product(
    ...
    list =>
      if (FOmega == cont(dom(list) :: list)) FOmega else FFail

  override def omega: (List[FTask]) => FTask = _ => FFail

  override def deFunction(
      index: Int,
      argument: (List[FTask]) => FTask,
      cont: (List[FTask]) => FTask
  ): (List[FTask]) => FTask =
    list =>
      get(index)(list) match {
        case FProduct(c, d) if argument(list) == c =>
          cont(d :: list)
        case _ => FFail
      }
}
...
object Valuing extends FExpr[FTask] {
  ...
  private def push(
    ...
    case FClosure(d, c) =>
      FClosure(push(value, d, depth), push(value, c, depth + 1))
    case FProduct(d, c) =>
      FProduct(push(value, d, depth), push(value, c, depth + 1))
    case FDefunction(i, a, c) if i < depth =>
      FDefunction(
        i,
        push(value, a, depth),
        push(value, c, depth + 1)
      )
    case FDefunction(i, a, c) if i == depth =>
      value match {
  ...
  override def function(dom: FTask, cont: FTask): FTask =
    FClosure(dom, cont)
  ...
  override def product(dom: FTask, cont: FTask): FTask =
    FProduct(dom, cont)

  override def omega: FTask = FOmega

  override def push(value: FTask, cont: FTask): FTask =
    push(value, cont, 0)
}
