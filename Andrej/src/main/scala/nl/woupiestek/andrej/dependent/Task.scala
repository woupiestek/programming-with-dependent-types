...
case class PushTask(value: Expression, stack: List[Task])
    extends Task {
  override def next: Option[Task] = value match {
    case Get(i) => stack lift i
    case Push(x, y) =>
      Some(PushTask(y, PushTask(x, stack) :: stack))
...
case class TypeTask(value: Expression, stack: List[Task])
    extends Task {
  override def next: Option[Task] = value match {
    case Get(i) => stack lift i
    case Push(x, y) =>
      Some(TypeTask(y, TypeTask(x, stack) :: stack))
    case Function(a, b) =>
      Some(TypeTask(b, PushTask(a, stack) :: stack)) //sticking with the 'no product' idea!
    case Product(a, b) =>
      Some(PushTask(b, PushTask(a, stack) :: stack))
    case Omega => None
    case Apply(x, y, z) =>
      for {
        tx <- stack lift x
...
case class InferenceTask(x: Task, y: Task) extends Task {
  //all is here...
  private def equal(ta: Task, ty: Task) =
    (ta.eval, ty.eval) match {
  ...
  private def all(list: List[(Task, Task)]): Boolean =
    list match {
      case Nil => true
      case (a, b) :: tail =>
        equal(a, b) match {
          case None        => false
          case Some(list2) => all(list2 ++ tail)
        }
    }

  override def next: Option[Task] = x match {
    case PushTask(Product(b, a), c) =>
      if (equal(y, PushTask(b, c)).exists(all))
        Some(PushTask(a, c))
      else None
