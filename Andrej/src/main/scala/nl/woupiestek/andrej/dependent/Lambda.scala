...
case class LClosure(dom: Lambda, fun: Lambda => Lambda)
    extends Lambda

case class LProduct(dom: Lambda, fun: Lambda => Lambda)
    extends Lambda
...
object LambdaValuing extends FExpr[List[Lambda] => Lambda] {
  override def get(index: Int): (List[Lambda]) => Lambda =
    _ lift index getOrElse LFail

  override def push(
      value: (List[Lambda]) => Lambda,
      cont: (List[Lambda]) => Lambda
  ): (List[Lambda]) => Lambda =
    list => cont(value(list) :: list)

  override def function(
      dom: (List[Lambda]) => Lambda,
      cont: (List[Lambda]) => Lambda
  ): (List[Lambda]) => Lambda =
...
object LambdaTyping extends FExpr[List[Lambda] => Lambda] {
  ...
  def unifies(x: Lambda, y: Lambda): Boolean = (x, y) match {
    ...
    case (LClosure(a, b), LClosure(c, d)) =>
      unifies(a, c) && unifies(b(LGet(0)), d(LGet(0)))
    case (LProduct(a, b), LProduct(c, d)) =>
      unifies(a, c) && unifies(b(LGet(0)), d(LGet(0)))
    case _ => false
  }
  ...
  override def deFunction(
    ...
    list =>
      get(index)(list) match {
        case LProduct(c, d) if unifies(argument(list), c) =>
          cont(d(argument(list)) :: list)
        case _ => LFail
      }
  ...
  override def product(
    ...
    list =>
      if (cont(dom(list) :: list) == LOmega) LOmega else LFail
