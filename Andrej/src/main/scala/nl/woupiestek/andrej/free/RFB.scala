...
object RFB {
  ...
  case class RUnit[F[_], A](a: A) extends RFB[F, A] {
    override def resume(
        implicit F: Monad[F]
    ): F[\/[RFB[F, A], A]] = F.unit(\/-(a))

    override def flatMap[B](f: (A) => RFB[F, B]): RFB[F, B] =
      f(a)
  }

  case class RBind[F[_], A0, A](fa: F[A0], g: A0 => RFB[F, A])
      extends RFB[F, A] {
    override def resume(
        implicit F: Monad[F]
    ): F[\/[RFB[F, A], A]] = F.map(fa)(a => -\/(g(a)))

    override def flatMap[B](f: (A) => RFB[F, B]): RFB[F, B] =
      copy(g = g(_: A0).flatMap(f))
  }

  def kata[F[_], A](
      implicit F: MonadRec[F]
  ): RFB[F, A] => F[A] = F.tailRec[RFB[F, A], A](_.resume)

  implicit def instance[F[_]]
      : MonadRec[({ type G[A] = RFB[F, A] })#G] =
    new MonadRec[({ type G[A] = RFB[F, A] })#G] {
      ...
      override def bind[A, B](fa: RFB[F, A])(
          f: (A) => RFB[F, B]
      ): RFB[F, B] = fa.flatMap(f)
