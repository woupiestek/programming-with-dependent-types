...
object BreakDown2 {
  ...
  case class Check(left: BreakDown2, right: BreakDown2, context: BreakDown2) extends BreakDown2
  ...
  def normalize(
    ...
    case Index(string) => heap.lift(string) match {
      case Some(Left(Task(a, b))) => normalize(a, b ++ heap, stack, eqs, arity)
      case Some(Right(index)) => (Right(index), stack, eqs, arity)//don't know what to do with these
      case None => (Left(string), stack, eqs, arity)
    }
    case Abs(b) => stack match {
      case Nil => normalize(b, Right(arity) :: heap, Nil, eqs, arity + 1) //moving inside bindings is dangerous...
