...
object X {
  ...
  object instance extends TermLike[String, Equations => Equations] {
    override def variable(id: String): Equations => Equations = e => e.copy(
      freeVariables = e.freeVariables + ((e.position, id)))

    override def lambda(id: String, body: Equations => Equations): Equations => Equations =
      e => {
        val f = body(e)
        val v = f.freeVariables.collect { case (x, y) if y == id => (x, f.position + 1) }
        f.copy(
          position = f.position + 2,
          freeVariables = f.freeVariables.filterNot(_._2 == id),
          sameValue = f.sameValue ++ v,
          sameType = f.sameType ++ v,
          arrows = f.arrows + ((f.position + 2, f.position + 1, e.position)),
    ...
    override def operate(operator: Equations => Equations, operand: Equations => Equations): Equations => Equations =
      e => {
        ...
        g.copy(
          ...
          arrows = g.arrows + ((f.position, g.position, g.position + 1)),
          applications = g.applications + ((g.position + 1, f.position, g.position)))
      }

    override def let(id: String, value: Equations => Equations, context: Equations => Equations): Equations => Equations =
      e => {
        val f = context(e)
        val g = value(f.copy(position = f.position + 1))
        val v = f.freeVariables.collect { case (x, y) if y == id => (x, f.position + 1) } + ((g.position, f.position + 1))
        g.copy(
          position = g.position + 1,
          freeVariables = g.freeVariables.filterNot(_._2 == id),
          sameValue = g.sameValue ++ v,
          sameType = g.sameType ++ v + ((g.position + 1, f.position)),
          lets = g.lets + ((g.position + 1, id, g.position, f.position)))
      }

    override def check(left: Equations => Equations, right: Equations => Equations, context: Equations => Equations): Equations => Equations =
      e => {
        val f = context(e)
        val g = left(f)
        val h = right(g)
        h.copy(
          ...
          sameValue = h.sameValue + ((f.position, g.position)),
          sameType = h.sameType + ((f.position, g.position)) + ((h.position + 1, f.position)),
          check = h.check + ((h.position + 1, f.position, g.position, h.position)))
      }
  }
