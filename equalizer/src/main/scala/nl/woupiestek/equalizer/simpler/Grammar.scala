...
class Grammar[P[_]: ApplicativePlus](readIf: (Char => Boolean) => P[Char]) {
  ...
  def term[T](T: TermLike[String, T]): P[T] =
    (name |@| list(mod(T)))((x, y) => y.foldLeft(T.variable(x))((a, b) => b(a)))

  private def mod[T](T: TermLike[String, T]): P[T => T] =
    ((keyword("be") *> name) |@| term(T))((x, y) => T.let(x, y, _)) <+>
      (keyword("for") *> name).map(x => T.lambda(x, _)) <+>
      ((keyword("if") *> term(T)) |@| (keyword("is") *> term(T)))(
        (x, y) => T.check(x, y, _)
      ) <+>
  ...
  def list[O](rule: P[O]): P[List[O]] = nel(rule) <+> List.empty[O].point[P]
