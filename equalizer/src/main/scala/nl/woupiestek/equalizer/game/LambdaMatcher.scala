...
object LambdaMatcher {
  ...
  final case class E(
    ...
    def lambda(key: String) = stack match {
      ...
      case Nil    => copy(heap = heap + (key -> fresh(depth)), depth = depth + 1)
    }
  ...
  final case class Q(varName: Either[Int, String], stack: List[Reader[E, Q]])
  ...
  implicit class Ops(val reader: Reader[E, Q]) {
    def closure: Reader[E, Reader[E, Q]] =
      Reader(e => reader.local(_.copy(heap = e.heap, stack = Nil)))
  }
  ...
  object PInstance {
    def application(
      ...
      operand.closure.flatMap((c: Reader[E, Q]) => operator.local(_.push(c)))
    def abstraction(varName: String, body: Reader[E, Q]): Reader[E, Q] =
      body.local(_.lambda(varName))
    def let(varName: String, value: Reader[E, Q], body: Reader[E, Q]) =
      value.closure.flatMap((c: Reader[E, Q]) => body.local(_.let(varName, c)))
  ...
  def equate(
    ...
    def traverse[X, Y](
        list: List[X]
    )(f: X => Reader[E, List[Y]]): Reader[E, List[Y]] = list match {
    ...
    def next(
      ...
      val fs = ql.stack ++ (e1.depth until e2.depth).map(fresh).toList
      if (i < 0) {
        traverse(
          fs.zip(qr.stack.drop(-i))
        ) { case (x: Reader[E, Q], y: Reader[E, Q]) => equate(x, y) }
          .map(
